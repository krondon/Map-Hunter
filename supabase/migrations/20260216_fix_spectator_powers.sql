-- Migration: Fix Spectator Power Logic (Integrity & Targeting)
-- Date: 2026-02-16
-- Purpose: 
-- 1. Fix "Friendly Fire": Spectators utilizing defense powers should apply them to the TARGET, not themselves.
-- 2. Fix "Defense Piercing": Ensure attacks validly check the target's protection status.
-- 3. Fix "Overwrite": Prevent applying defense if target is already protected.
...
DECLARE
    ...
    v_caster_is_protected boolean; -- Not used for defense logic anymore, but kept for legacy/safety if needed
...
BEGIN
    ...
    -- Spectators/Players pay for the power
    ...
    --    CRITICAL FIX: Apply logic to p_target_id, NOT p_caster_id
    IF p_power_slug IN ('shield', 'return', 'invisibility') THEN
        
        -- Get Target's Protection Status (The intended beneficiary)
        ...
        -- CRITICAL FIX: target_id is p_target_id. caster_id remains p_caster_id (who paid).
        ...
        RETURN json_build_object(
            ...
            'target_id', p_target_id -- Return for confirmation
        );
    END IF;
    ...
    -- BUT if they were targeted (e.g. latency), we check and refund.
    ...
    IF v_returned THEN
        -- Life Steal logic handling for reflection: Original Attacker (now Target) loses life, Original Defender (now Caster) gains life
